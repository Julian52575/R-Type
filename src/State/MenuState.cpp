...
namespace RType {
    ...
    {
        this->initScenes();
        this->_sceneManager.setScene(RTypeMenuScenesButtonDisplay);
        // Background music
    ...
    State buttonDisplay(MenuState& thiss)
    {
        ...
        // _cursor
        Rengine::Graphics::GraphicManagerSingletone::get().addToRender(thiss._cursor, {50, yDiff * (int) (thiss._currentIndex + 1) + 15 });
        // Display button and input
        for (auto it : thiss._buttonVector) {
            ...
            if (it.type == Rengine::Graphics::UserInputTypeKeyboardSpecialPressed) {
                switch (it.data.keyboardSpecial) {
                    ...
                    case Rengine::Graphics::UserInputKeyboardSpecialENTER:
                        this->_sceneManager.setScene(RTypeMenuScenesLoadLobbyInfoAndExit);
                        break;
    ...
    State loadLobbyInfoAndExit(MenuState& thiss)
    {
        thiss._lobbyInfo.serverIp = thiss._buttonVector[MenuStateButtonsIp].second->getText();
        try {
            thiss._lobbyInfo.port = std::stoi(thiss._buttonVector[MenuStateButtonsPort].second->getText());
        } catch (std::exception& e) {
            std::cout << "Exception '" << e.what() << "' when converting port input to int. Staying on menu." << std::endl;
            thiss._sceneManager.setScene(RTypeMenuScenesButtonDisplay);
            return StateMenu;
        }
        thiss._backgroundMusic->reset();
        return StateLobby;
    }
    ...
    void MenuState::initScenes(void)
    {
        ...
        this->_sceneManager.setSceneFunction<State, MenuState&>(RTypeMenuScenesButtonDisplay, buttonDisplayFun);
        this->buttonDisplayInitButtons();
        std::function<State(MenuState&)> loadLobbyInfoFun(loadLobbyInfoAndExit);

        this->_sceneManager.setSceneFunction<State, MenuState&>(RTypeMenuScenesLoadLobbyInfoAndExit, loadLobbyInfoFun);
    }
