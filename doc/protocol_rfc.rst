...
- Connexion info:
    ...
    5, PlayableEntityInGameId   , The server tells the client what entity the client's player controls    , In-Game Entity Id           , 2;
    6, ServerStop               , The server is stopping                                                  , None;
    7, RequestPlaybleEntity,      The client is asking for the id of his playable entity                  , None;
...
- Action:
    0, Move             , Move (Velocity x and y between -100 and 100)          , x + y         , 2 + 2;
...
- Info:
    ...
    8, NewEntity            , New entity created                                    , Configuration Id + In-Game Id + X + Y            , 2 + 2 + 2 + 2;
...
This example is based on an already playing game that has 2 entity.
Client                   |                      Server
------------------------------------------------------
 [
 ConnexionDetail,
...
3.3 In-Game event handling
---------------------------

[ Every refresh time ]
The server sends EntityInfo.Info.InfoAll for each entity to every client.

[ Entity action ]
    [ Player Action ]
    The client send EntityAction.Action.<currentAction> whenever the player inputs an recognized action.

    [ Enemy Action ]
    ...

[ Colision between 2 entities ]
...

[ Entity death ]
...

3.4 Server / Client differences
--------------------------------
...
[ Client ]
The client should be seen as a façade, only displaying what the server tells him to.
In case of desync, the game, hopefully, stays only desynced until refresh time when the server resends all the EntityInfo.
Computations on damage dealt or even patern prediction can be made on the client but isn't necessary.
...
- Version 1.1: Added strength && Updated Move* with strength && Added InfoAll. 6 december 2024. 17:10UTC+1
- Version 1.2: Concat all 4 move precision into a single one with X & Y. 7 december 2024. 16:01UTC+1
- Version 1.3: Removed Entity In-Game id from all Precision::Action. 7 decembre 2024. 16:07UTC+1
